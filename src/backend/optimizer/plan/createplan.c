...
create_gather_plan(PlannerInfo *root, GatherPath *best_path)
{
	...
	ereport(WARNING, (errmsg("[DEBUG/문태주] Gather Plan 구성중...")));
	/*
...
create_unique_plan(PlannerInfo *root, UniquePath *best_path, int flags)
{
	...
	plan = make_unique_from_pathkeys(subplan,
									 ...
									 best_path->numkeys,
									 IS_OTHER_REL(best_path->path.parent) ? best_path->path.parent->relids : NULL);
					...
					List *tlist, List *scan_clauses)
{
	...
	RangeTblEntry *rte = planner_rt_fetch(best_path->parent->relid, root);
	ereport(WARNING, (errmsg("[DEBUG/문태주] Seq Scan [%s]", rte->eref->aliasname)));
						...
						List *scan_clauses)
{
	...
	RangeTblEntry *rte = planner_rt_fetch(best_path->path.parent->relid, root);
	ereport(WARNING, (errmsg("[DEBUG/문태주] 비트맵 인덱스 스캔 [%s]", rte->eref->aliasname)));
					...
					RelOptInfo *rel)
{
	...
	plan->lefttree = NULL;
	plan->righttree = NULL;
	node->result_type = IS_UPPER_REL(rel) ? RESULT_TYPE_UPPER : IS_JOIN_REL(rel) ? RESULT_TYPE_JOIN
																				 : RESULT_TYPE_SCAN;
	node->resconstantqual = resconstantqual;
